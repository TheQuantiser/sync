...
options:
  -d DIR      scan DIR                  [default: ~/.ssh]
  -m SUBSTR   only consider paths matching SUBSTR
  -x SUBSTR   exclude paths matching SUBSTR; repeatable
  -t SEC      ssh control timeout       [default: 2]
  -n          dry run
  -f          force prune weird mux sockets
  -i          interactive rm
  -a          include non-mux sockets in info output
  -v          verbose; include raw conn/channels hints
  -q          quiet; suppress trailer line
  -h          help
USAGE
}
...
_mux_ps_meta() {
  ...
  ps -o etime= -o lstart= -o args= -p "$pid" 2>/dev/null | sed 's/^[[:space:]]*//'
}
...
_mux_cmd_parse() {
  local pid=$1
  local arg expect host user port cfg jump
...
_mux_lsof_peer() {
  ...
  out=$(lsof -Pan -p "$pid" -iTCP 2>/dev/null | awk 'NR>1 && $9 ~ /->/ {print $9; exit}') || return 1
  [ -n "$out" ] || return 1
  case $out in
    *'->'*) printf '%s\n' "${out#*->}" ;;
    *)      printf '%s\n' "$out" ;;
...
_mux_ss_peer() {
  ...
  out=$(ss -ntp 2>/dev/null | awk -v pid="$pid" '$0 ~ ("pid=" pid ",") {print $5; exit}') || return 1
  [ -n "$out" ] || return 1
  printf '%s\n' "$out"
}
...
_mux_peer() {
  local pid=$1 out
  out=$(_mux_lsof_peer "$pid") && { printf '%s\n' "$out"; return 0; }
  out=$(_mux_ss_peer "$pid")   && { printf '%s\n' "$out"; return 0; }
  return 1
}

_mux_channels() {
  local sock=$1 timeout_s=${2:-2} out
  out=$(_mux_sshctl "$sock" channels "$timeout_s") || return 1
  printf '%s\n' "$out"
}
...
_mux_conninfo() {
  local sock=$1 timeout_s=${2:-2} out
  out=$(_mux_sshctl "$sock" conninfo "$timeout_s") || return 1
  printf '%s\n' "$out"
}
...
_mux_config_persist() {
  local host=$1 user=$2 port=$3 cfg=$4 out
  local -a args
  ...
  out=$(ssh "${args[@]}" 2>/dev/null) || return 1
  printf '%s\n' "$out" | awk '$1=="controlpersist"{print $2; exit}'
}
...
_mux_enrich() {
  local sock=$1 pid=$2 timeout_s=$3
  local psline etime lstart args
  local host user port cfg jump
  local peer conn channels chcount persist desc
  local key val

  psline=$(_mux_ps_meta "$pid" 2>/dev/null || true)
  if [ -n "$psline" ]; then
    etime=$(printf '%s\n' "$psline" | awk '{print $1}')
    lstart=$(printf '%s\n' "$psline" | awk '{print $2, $3, $4, $5, $6}')
    args=$(printf '%s\n' "$psline" | awk '{for (i=7; i<=NF; ++i) printf("%s%s", $i, (i<NF?" ":"")); printf("\n")}')
  fi
  ...
  done < <(_mux_cmd_parse "$pid" 2>/dev/null)

  desc=$(_mux_desc "$user" "$host" "$port")
  peer=$(_mux_peer "$pid" 2>/dev/null || true)
  conn=$(_mux_conninfo "$sock" "$timeout_s" 2>/dev/null || true)
  channels=$(_mux_channels "$sock" "$timeout_s" 2>/dev/null || true)
  chcount=$(_mux_channels_count "$channels")
  persist=$(_mux_config_persist "$host" "$user" "$port" "$cfg" 2>/dev/null || true)
  ...
  printf 'jump\t%s\n'       "${jump:-}"
  printf 'argv\t%s\n'       "${args:-}"
  printf 'connraw\t%s\n'    "$(_mux_first_line "$conn")"
  printf 'channelsraw\t%s\n' "$(_mux_first_line "$channels")"
}

_mux_rm() {
  local sock=$1 dry=$2 interactive=$3
  if [ "$dry" -eq 1 ]; then
    return 0
  fi
...
_mux_scan() {
  local dir=$1
  find "$dir" -xdev -type s -user "$(id -un)" -print 2>/dev/null
}
...
muxctl() {
  ...
  local dir timeout_s dry_run force interactive show_nonmux verbose quiet
  local match sock short state pid note
  local age desc peer channels persist start connraw channelsraw
  local key val
  local -a excludes inputs
  local -a live_rows stale_rows hung_rows weird_rows nonmux_rows foreign_rows
  local -a closed_rows pruned_rows kept_rows
  ...
  while [ $# -gt 0 ]; do
    case $1 in
      -d) [ $# -ge 2 ] || { printf 'muxctl: -d needs arg\n' >&2; return 64; }; dir=$2; shift 2 ;;
      -m) [ $# -ge 2 ] || { printf 'muxctl: -m needs arg\n' >&2; return 64; }; match=$2; shift 2 ;;
      -x) [ $# -ge 2 ] || { printf 'muxctl: -x needs arg\n' >&2; return 64; }; excludes+=("$2"); shift 2 ;;
      -t) [ $# -ge 2 ] || { printf 'muxctl: -t needs arg\n' >&2; return 64; }; timeout_s=$2; shift 2 ;;
      -n) dry_run=1; shift ;;
      -f) force=1; shift ;;
      -i) interactive=1; shift ;;
      -a) show_nonmux=1; shift ;;
      -v) verbose=1; shift ;;
      -q) quiet=1; shift ;;
      -h|--help) _mux_usage; return 0 ;;
      --) shift; while [ $# -gt 0 ]; do inputs+=("$1"); shift; done ;;
      -*) printf 'muxctl: bad option: %s\n' "$1" >&2; return 64 ;;
      *)  inputs+=("$1"); shift ;;
    esac
  done

  case $timeout_s in
    ''|*[!0-9]*) printf 'muxctl: -t expects integer seconds\n' >&2; return 64 ;;
  esac

  if [ ${#inputs[@]} -eq 0 ]; then
    while IFS= read -r sock; do
      [ -n "$sock" ] || continue
      _mux_is_excluded "$sock" "${excludes[@]}" && continue
      if [ -n "$match" ]; then
        case $sock in *"$match"*) ;; *) continue ;; esac
      fi
      inputs+=("$sock")
    done < <(_mux_scan "$dir")
  fi
...
EOF
    ...
    case $cmd in
      ls|list)
        [ "$state" = live ] || continue
        age='?'; desc='?'; peer='?'; channels='?'; persist='unknown'
        while IFS=$'\t' read -r key val; do
          case $key in
            age)      age=$val ;;
            desc)     desc=$val ;;
        ...
        done < <(_mux_enrich "$sock" "$pid" "$timeout_s")

        live_rows+=("$(printf '%-7s  %-10s  %-30s  %-22s  ch=%-3s  cp=%-8s  %s' \
          "$pid" "$age" "$desc" "$peer" "$channels" "$persist" "$short")")
        ;;

      info)
        case $state in
          live)
            age='?'; start='?'; desc='?'; peer='?'; channels='?'; persist='unknown'; connraw=''; channelsraw=''
            while IFS=$'\t' read -r key val; do
              case $key in
                age)         age=$val ;;
                start)       start=$val ;;
                ...
                persist)     persist=$val ;;
                connraw)     connraw=$val ;;
                channelsraw) channelsraw=$val ;;
              esac
            done < <(_mux_enrich "$sock" "$pid" "$timeout_s")

            if [ "$verbose" -eq 1 ]; then
              live_rows+=("sock=$short
pid=$pid age=$age start=$start
dst=$desc
peer=$peer
mux=channels:$channels persist:${persist:-unknown}
conn=${connraw:-?}
chan=${channelsraw:-?}")
            else
              live_rows+=("sock=$short
...
mux=channels:$channels persist:${persist:-unknown}")
            ...
            ;;
          stale)   stale_rows+=("sock=$short
state=stale") ;;
          hung)    hung_rows+=("sock=$short
state=hung
note=$note") ;;
          weird)   weird_rows+=("sock=$short
state=weird
note=$note") ;;
          foreign) foreign_rows+=("sock=$short
state=foreign") ;;
          nonmux)  [ "$show_nonmux" -eq 1 ] && nonmux_rows+=("sock=$short
state=nonmux
note=$note") ;;
        esac
        ;;
      ...
      close)
        ...
        if [ "$dry_run" -eq 1 ]; then
          ...
          if _mux_sshctl "$sock" exit "$timeout_s" >/dev/null; then
            closed_rows+=("$short")
          else
            kept_rows+=("$short  exit-failed")
          fi
        fi
        ;;
      ...
      prune)
        case $state in
          stale|hung)
            if _mux_rm "$sock" "$dry_run" "$interactive"; then
              pruned_rows+=("$short")
            else
              kept_rows+=("$short  rm-failed")
            fi
            ;;
          weird)
            if [ "$force" -eq 1 ]; then
  ...
  case $cmd in
    ...
    close|clean|prune)
      ...
      [ "$quiet" -eq 1 ] || printf 'done  scan %d  closed %d  pruned %d  kept %d\n' \
        "${#inputs[@]}" "${#closed_rows[@]}" "${#pruned_rows[@]}" "${#kept_rows[@]}"
      ;;
  esac
}
...
expect {
    -re {\([^)]+@[^)]+\)\s+Password:\s*$} {
        incr password_prompt_count
        if {$password_prompt_count <= 2} {
            send -- "\r"
            exp_continue
        } else {
            puts stderr "\nlxplus_auto: repeated Password prompts before 2FA."
            child_exit $ssh_spawn_id
        }
    }
    ...
    -re {Your 2nd factor \([^)]+\):\s*$} {
        ...
        set saved_tty [exec stty -g < /dev/tty]
...
fixtime() {
    echo "── Resyncing system clock ──"

    # 1. Disable NTP so we can manually set time
    sudo timedatectl set-ntp false || { echo "✗ Failed to disable NTP"; return 1 }

    # 2. Bootstrap time from HTTP Date header — works even when RTC is wildly wrong.
    #    Try a few reliable hosts in order.
    local net_time
    for host in google.com cloudflare.com time.cloudflare.com; do
        net_time=$(
            curl -sI --max-time 5 "$host" 2>/dev/null \
            | grep -i '^date:' \
            | sed 's/[Dd]ate: //' \
            | xargs -I{} date -d "{}" '+%Y-%m-%d %H:%M:%S' 2>/dev/null
        )
        [[ -n "$net_time" ]] && break
    done

    if [[ -n "$net_time" ]]; then
        echo "  Network time : $net_time"
        sudo timedatectl set-time "$net_time" || { echo "✗ Failed to set time"; return 1 }
        echo "  System clock set ✓"
    else
        echo "  ⚠ Could not fetch network time — NTP will attempt to correct on its own"
    fi

    # 3. Write system time → hardware clock immediately
    sudo hwclock --systohc
    echo "  Hardware clock synced ✓"

    # 4. Re-enable NTP and restart timesyncd for fine-grained correction
    sudo timedatectl set-ntp true
    sudo systemctl restart systemd-timesyncd

    # 5. Wait for NTP confirmation (up to 30s)
    ...
    if (( elapsed >= 30 )); then
        echo "  ⚠ NTP sync timed out — service may still catch up in the background"
    else
        echo "  NTP synchronized ✓ (${elapsed}s)"
        sudo hwclock --systohc   # write the NTP-corrected time to RTC too
    fi
